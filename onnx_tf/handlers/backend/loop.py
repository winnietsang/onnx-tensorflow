...
class Loop(BackendHandler):
  ...
  def _common(cls, node, **kwargs):
    ...
    def run_subgraph(iter_cnt, cond, v, scan_outputs):
      tensor_dict = kwargs["tensor_dict"]
      tensor_dict[body.input[0].name] = iter_cnt
      tensor_dict[body.input[1].name] = cond
      for i in range(2, len(body.input)):
        tensor_dict[body.input[i].name] = v[i - 2]
      tensor_dict = onnx_tf.backend.onnx_graph_to_tensorflow_ops(
          subgraph=body, tensor_dict=tensor_dict, opset=current_opset)
    ...
    if scan_outputs_start_index == len(body.output):
      ...
      # if the loop has run at least 1 time then do nothing
      def true_fn():
